...
function initGmag() {
   ...
   $('.show_search-btn i').click(function() {
      $(this).parent().toggleClass("htact")
      $('.header-search-wrap').fadeToggle(200)
   });
...
let store = $("#store").val();
if (store) {
   let alpha = ['all stores', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
   $(alpha).each(function(index, el) {
      $(".style-select").append(`<option>Store From ${el.toUpperCase()}</option>`);
   });
   $(".nice-select").ready(function() {
      var setLink = $(this).find('.list').children('li.option');
      $(setLink).each(function(index, el) {
         $(this).text(" ").append(`<a href="#">${el.getAttribute("data-value")}</a>`);
      });
   })
}